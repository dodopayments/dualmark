...
<Tabs items={["Astro", "Nuxt", "Next.js", "SvelteKit", "Vercel", "Cloudflare", "Deno"]}>
<Tab value="Astro">
...
Nuxt also code-generates server routes at build time, so closures can't be serialized. Pass a **module path** instead:
...
The generated handlers will `import` your module directly — no serialization issues with closed-over encoders.
