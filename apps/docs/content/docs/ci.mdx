...
| **Artifact upload** | `aeo.json` saved as `aeo-report` — download in later steps or other jobs |
...
<Callout type="info">
  The action automatically uploads `aeo.json` as a build artifact (`aeo-report`). Use
  `actions/download-artifact@v4` in downstream jobs.
</Callout>
