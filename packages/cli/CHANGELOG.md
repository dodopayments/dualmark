...
- 65ab17e: Stabilize `dualmark verify --json` around an AEO Spec v1.0 JSON contract and enforce `--json` mutual exclusivity with `--quiet`/`--color`, while keeping required-check failures as non-zero exits.
