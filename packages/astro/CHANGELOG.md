...
- 382d05e: Include the config file path in Dualmark config validation error messages (e.g. `[astro.config.mjs] Dualmark config error: siteUrl is required`), making it clear which file to fix when the integration is misconfigured.
...
- 0cf13c4: Bump `astro` peer dependency to `^6.1.10` (was `^5.0.0`) to resolve two upstream
